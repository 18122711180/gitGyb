    ...
    <section>
      <template v-if="type == 1">
        <div class="order-success flex-start-center">
          ...
          <p class="flex-start-start">订单提交成功，请在<span>26分46秒</span>内完成支付！（逾期将自动取消订单）</p>
        </div>
      </template>
      <template>
      ...
      <div class="order-detail-message">
        <p>订单编号： <span>{{orderCode}}</span></p>
        <p>下单时间： <span>{{orderTime}}</span></p>
        <p>收货人姓名： <span>{{orderName}}</span></p>
        <p>联系电话： <span>{{orderTel}}</span></p>
        <p>收货地址： <span>{{orderAddress}}</span></p>
      </div>
