      ...
      <div class="login-left">
        ...
        <div class="login-left-main flex-start-center flex-column">
          <div class="login-left-step flex-start-start">
            <p
              class="flex-start-center flex-column"
              :class="{on : stepType == index}"
            ...
            <el-form ref="form" :model="form" label-width="90px">
              ...
              <div class="login-left-checked flex-start-start">
                ...
                <div class="login-left-submit" @click="submit1">
                  <span style="cursor: pointer;">下一步</span>
                </div>
              </div>
            </el-form>
          </div>
          ...
          <div class="login-left-step-2 flex-start-center flex-column" v-else-if="stepType == 1">
            <div class="login-left-step-2-tis">
              <!-- <p>
                共医保已向您的手机
                <span>{{myTel}}</span> 发送了短信验证码，请及时查收！
              </p> -->
            </div>
...
export default {
  ...
  methods: {
    ...
    submit2: function() {
      var that = this;
      this.post("/yiqi-api/api/User/CodeIsExist", {
        mobile: that.form.tel,
        smscode: that.form2.code
      })
        .then(res => {
          console.log(res);
          if (res.msg == 1) {
    ...
    submit3: function() {
      ...
      if (
        that.form3.newPsd === "" ||
        that.form3.newPsd.length < 8 ||
        that.form3.newPsd.length > 16
      ) {
        this.$message.error("密码长度为8到16为字符");
        return false;
      }
      if (that.form3.newPsd !== that.form3.psdSure) {
        this.$message.error("两次输入密码不一致");
        return false;
      }
      this.post("/yiqi-api/api/User/ForgotPwd", {
        mobile: that.form.tel,
        password: that.form3.newPsd
      })
        .then(res => {
          that.stepType = 3;
        })
        .catch(err => {
          this.$message.error("新密码有误");
        });
    },
    getCode: function() {
      var that = this;
