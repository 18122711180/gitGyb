<template>
  <div id="food">
    ...
    <section class="flex-between-start">
      <div class="notice-menu flex-start-start flex-column omit">
        <span
      ...
      <div class="notice-list">
        <router-link
          v-for="(item, index) in communityList[noticeMenuType].list"
          :key="index"
          :to="{ path: '/goods/detail', query: { id: item.pid , menu: login.menu}}"
        >
          <!-- /goods/detail?id=1&menu=5 -->
          <!-- :to="{ path: '/community/detail', query: { id: item.id }}" -->
...
export default {
  ...
  methods: {
    getHealth() {
      ...
      this.post("/yiqi-api/api/store/healthlist", v)
        .then(res => {
          console.log(res);
