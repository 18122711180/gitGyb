        ...
        <div class="order-frame">
          <div class="order-menu flex-start-center">
            <span
              ...
              @click="orderMenuType = index; getdetail()"
              :key="index"
            >{{item}}</span>
          </div>
          <div class="collect">
            <template v-if="orderMenuType == 0">
              <goods-list :list="collect"></goods-list>
            </template>
            ...
            <template v-else-if="orderMenuType == 2 || orderMenuType == 3">
                ...
                >
                  ...
                  <div class="doctor-list-main">
                    <div class="doctor-list-basic flex-start-center">
                      ...
                      <span v-if="orderMenuType == 3" style="style='color: #ff4a60'">护理员</span>
                      <!-- <span
                        class="float-doctor-follow"
                        :class="{on : item.state}"
                      >{{item.state ? "在线" : "离线"}}</span> -->
                    </div>
...
export default {
  ...
  methods: {
    getdetail() {
      var url = "";
      switch (this.orderMenuType){
        case 0: 
        url = '/yiqi-api/api/perother/bclist';
        break;
        case 1: 
        url = '/yiqi-api/api/perother/bflist';
        break;
        case 2: 
        url = '/yiqi-api/api/perother/fllowyh';
        break;
        case 3: 
        url = '/yiqi-api/api/perother/fllowhg';
        break;
      }
      this.post(url, { token: this.userInfo.token })
        .then(res => {
          this.collect = res.data
          if(this.orderMenuType == 0){
             this.collect.map(value => {{
               value.shopprice = value.price
             }})
          }
